...
void MainWindow::on_openfile_clicked()
{
    ...
    if (mime.name().startsWith("image/"))
    {
        ...
        Mat temp;
        if(src.channels()==4)
            cvtColor(src,temp,COLOR_BGRA2RGB);
        else if (src.channels()==3)
            cvtColor(src,temp,COLOR_BGR2RGB);
        else
            cv::cvtColor(src,temp,COLOR_GRAY2RGB);
        ...
        QImage img = QImage((uchar*)(temp.data),temp.cols,temp.rows,temp.step,QImage::Format_RGB888);//Mat转QImage
